...
### Кэш статуса — СДЕЛАНО (post-MVP §3)
`StatusCache` (`cache/StatusCache.kt`) хранит последний успешный `RepoStatus` на
репозиторий в JSON-файле рядом с конфигом (не историю/тренды — только последнее
значение). `DashboardService.portfolioStatus()` мержит свежий опрос с кэшем через
чистую функцию `mergeWithCache` — **не** внутри `GitHubClient`/`GhCliClient`, кэш
ортогонален способу получения статуса. Инварианты, добытые ревью — не ломать:
- **Запись только атомарная** — temp-файл + `Files.move(..., ATOMIC_MOVE)`, не
  `Files.writeString` напрямую в целевой путь. Диагностический поток при старте
  веб-режима и первый запрос `/api/status` оба зовут `portfolioStatus()` почти
  одновременно — неатомарная запись гонится и может испортить файл.
  `portfolioStatus()` из-за этого **не pure read** — пишет на диск на каждый вызов.
- Живой сбой + есть кэш → `stale = true`, но **живая** ошибка (`RepoStatus.error`)
  обязана остаться видимой пользователю и в CLI, и в веб-UI (не просто в поле
  данных) — иначе деградация происходит незаметно.
- `GhDiagnostics.hint()` не должен считать `stale`-репозиторий «недоступным»
  (`it.error == null || it.stale`) — иначе штатная деградация в кэш ложно
  диагностируется как «gh не работает».
- Не заводить TTL/возраст кэша/историю — явно вне рамок этого этапа
  (см. `docs/plans/post-mvp-status-cache.md`, раздел «Не входит в эту задачу»).
